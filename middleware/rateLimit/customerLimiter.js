...
const WINDOW_DURATION_IN_MINUITS = 1;
...
const customLimiter = (req, res, next) => {
  try {
    ...
    // Gets the records of the current user base on the IP address,
    // returns a null if the is no user found
    redis_client.get(req.ip, (error, record) => {
      if (error) throw error;
      const currentTime = moment();

      //   When there is no user record then a new record is
      //   created for the user and stored in the Redis storage
      if (record == null) {
        let newRecord = [];
        let requestLog = {
      ...
      //When the record is found then its value is parsed and the number of requests the user has made
      //  within the last window is calculated
      let data = JSON.parse(record);
      let windowBeginTimestamp = moment()
        .subtract(WINDOW_DURATION_IN_MINUITS, "minutes")
        .unix();

      let requestsinWindow = data.filter((entry) => {
        return entry.requestTimeStamp > windowBeginTimestamp;
      });
      console.log("Requests_in_Window :", requestsinWindow);
      ...
      if (totalWindowRequestsCount >= MAX_WINDOW_REQUEST_COUNT) {
        res
          .status(429)
          .send(
            `You have exceeded the ${MAX_WINDOW_REQUEST_COUNT} requests in ${WINDOW_DURATION_IN_MINUITS} mins limit!`
          );
      } else {
        //When the number of requests made are less than the maximum the a new entry is logged
        let lastRequestLog = data[data.length - 1];
        let potentialCurrentWindowIntervalStartTimeStamp = moment()
